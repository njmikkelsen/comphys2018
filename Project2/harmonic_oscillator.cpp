...
int main(int argc, char* argv[])
{
  ...
  const double xi_inf  = (argc > 2) ? atof(argv[2]) : 4;     // approximation of infinity
  const int    N_max   = (argc > 3) ? atoi(argv[3]) : 100;   // maximum number of iterations
  const double epsilon = (argc > 4) ? atof(argv[4]) : 1e-8;  // max element error tolerance
  
  // print parameters to file
  ofstream File("HarmonicOscillator/"+to_string(n)+"_"+to_string((int)(xi_inf*10.))+".dat", ios::trunc);
  ios_base::fmtflags f(cout.flags());
  cout.flags(f);
  File << "Numerical Analysis Of A Quantum Harmonic Oscillator\n\n";
